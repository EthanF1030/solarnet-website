  ...
  return (
    <header className="fixed left-0 top-0 z-50 w-full border-b border-white/10 bg-black/60 backdrop-blur-xl">
      ...
      <nav className="relative flex h-20 w-full items-center justify-between px-6 md:px-10">
        ...
        {/* Desktop nav */}
        <div className="absolute left-1/2 hidden -translate-x-1/2 items-center rounded-full border border-white/10 bg-white/[0.04] px-2 py-2 shadow-[0_0_30px_rgba(0,0,0,0.35)] backdrop-blur-md md:flex">
          {navItems.map((item) => {
        ...
        <button
          ...
          className="flex h-11 w-11 items-center justify-center rounded-xl border border-cyan-400/30 bg-cyan-400/10 text-cyan-300 transition hover:bg-cyan-400 hover:text-black md:hidden"
      ...
      {/* Mobile dropdown */}
      {menuOpen && (
        <div className="border-t border-white/10 bg-black/90 px-6 pb-6 pt-4 backdrop-blur-xl md:hidden">
